		...
		if (proc && (SELECTION == NFU)){
			for (i = 0; i < MAX_PSYC_PAGES; i++){
				if (proc->pages.memory.pageTables[i].used == PAGE_USED){
					page = walkpgdir(proc->pgdir,(char*) proc->pages.memory.pageTables[i].virtualAddress,0);
					proc->pages.memory.pageTables[i].age = proc->pages.memory.pageTables[i].age >> 1;	//reduce weight of the past
					test = *page & PTE_A;
					if (test){	//check if reference bit is on
						*page = *page & ~PTE_A;		//turn reference off
						proc->pages.memory.pageTables[i].age += GROW;
					}
				}
			}
		}
