...
#define maxTime 1<<30   //Delete after test
...
scheduler(void)
{
	...
	for(;;){
		...
		for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
			if (p->state == RUNNABLE && (p->ctime < best->ctime || gotFirst==0)) {
				best=p;
				gotFirst=1;
			}
		}
	...
	//}
	 ...
	 struct proc *p;
	   int firstTime, highestPriority = 0;
	  ...
	  for(;;){
	    ...
	    // Loop over process table looking for process to run.
	    acquire(&ptable.lock);
	    firstTime = maxTime;//ptable.proc->enterTime;
	    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
	      if(p->state != RUNNABLE)
	        continue;

	     if(p->priority >= highestPriority && p->enterQTime < firstTime){
	       highestPriority = p->priority;
	       firstTime = p->enterQTime;
